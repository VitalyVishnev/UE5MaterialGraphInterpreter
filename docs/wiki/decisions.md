...
Expose the two vertical seams and the row seam between Clipboard and Graph Inspector as draggable separators. Resizing one panel redistributes the existing workspace rather than creating gaps. Constrain both side panes and the code column from one width budget so dragging either side cannot collapse the other to zero; snap a separator when it approaches its computed default, persist custom dimensions in `sessionStorage`, and restore one separator to default on double-click. Keyboard arrows resize a focused separator. On screens below `900px`, stack Clipboard, pseudo-HLSL, Code Settings, then Graph Inspector and hide the separators. Offer a native `Paste clipboard` action in addition to ordinary paste; analyze either path immediately.

Keep type-certainty legend above generated code. A compact always-visible code-search field highlights case-insensitive literal matches as the author types; clearing it removes search state. `Ctrl+F` focuses the field. Its active/total count and previous/next controls appear only for a non-empty query, so the idle toolbar stays compact. Optional line numbers use a stable left gutter and label every logical generated line, including blanks; visual wrapping does not fabricate source lines. Both are local presentation controls and never affect generated text or copied code.
...
Arithmetic inference treats a finite serialized default on an unconnected Math input as confirmed scalar evidence. This keeps loaded Material Function definitions consistent with standalone generation: operations such as `x * 0.5` propagate their output type even though only one input has a graph link. A confirmed Type Fact always outranks an incompatible inferred consumer constraint; two equally strong incompatible facts still remain an explicit conflict. The same rule applies to `dot(vector, scalar)`: the scalar broadcasts, while `dot` proves only its scalar result and does not force both operands to one width. `TransformPosition` is registered separately from direction-vector `Transform`; it accepts and returns `float3`, accepts a scalar Periodic World Tile Size, and renders its position-space modes explicitly.

`SceneTexture` output dimensions and `Desaturation`'s same-as-input relation are Registry contracts, not generator-only knowledge. This keeps standalone rendering and Function Definition Library type propagation on the same semantic source.
